...
/*
ADHOC: CSV: GET ~/reports/daily_group_sales/adhoc/csv?first=...&max=...&sort=...&filter=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&relativeTimeRange.reference=...
ADHOC: JSON: GET ~/reports/daily_group_sales/adhoc/json?first=...&max=...&sort=...&filter=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&relativeTimeRange.reference=...
*/
/*
CREATE: PUT ~/reports/daily_group_sales?sort=...&filter=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&name=...
UPDATE: PUT ~/reports/daily_group_sales/{id}?sort=...&filter=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&name=...
LIST: GET ~/reports/daily_group_sales
LOAD: GET ~/reports/daily_group_sales/{id}
DOWNLOAD REPORT DATA: JSON: GET ~/reports/daily_group_sales/{id}/json?first=...&max=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&relativeTimeRange.reference=...
DOWNLOAD REPORT DATA: CSV: GET ~/reports/daily_group_sales/{id}/csv?first=...&max=...&timeInterval.start=...&timeInterval.end=...&relativeTimeRange.code=...&relativeTimeRange.reference=...
...
public class DailyGroupSalesReport
{
	public static enum TransactionType implements Transaction.IType
	{
		...
		SELF_TOPUP(Transaction.Type.SELF_TOPUP),
		SELL_BUNDLE(Transaction.Type.SELL_BUNDLE),
		REVERSE(Transaction.Type.REVERSE),
		NON_AIRTIME_DEBIT(Transaction.Type.NON_AIRTIME_DEBIT),
		NON_AIRTIME_REFUND(Transaction.Type.NON_AIRTIME_REFUND);
		//ADJUDICATE(Transaction.Type.ADJUDICATE);
	...
	public static class TypeFactory implements Transaction.ITypeFactory
	{
		@Override
		public Class<TransactionType> getTypeClass()
		{
			return TransactionType.class;
		}

		@Override
		public TransactionType fromCode(String code) throws NullPointerException, IllegalArgumentException
		{
	...
	protected DailyGroupSalesReport()
	{
	}
}
