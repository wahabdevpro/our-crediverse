package hxc.connectors.smpp.client;
...
public class SmppClient implements com.cloudhopper.smpp.SmppClient
{
	final static Logger logger = LoggerFactory.getLogger(SmppClient.class);
	...
	/**
	 * Creates a new default SmppClient. Window monitoring and automatic expiration of requests will be disabled with no monitorExecutors. The maximum number of IO worker threads across any client
	 * sessions created with this SmppClient will be Runtime.getRuntime().availableProcessors(). An Executors.newCachedDaemonThreadPool will be used for IO worker threads.
	 */
	public SmppClient()
	{
	...
	/**
	 * Creates a new default SmppClient. Window monitoring and automatic expiration of requests will be disabled with no monitorExecutors. The maximum number of IO worker threads across any client
	 * sessions created with this SmppClient will be Runtime.getRuntime().availableProcessors().
	 * 
	 * @param executor
	 *            The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is recommended. The max threads will never grow more than expectedSessions if NIO sockets
	 *            are used.
	 */
	public SmppClient(ExecutorService executors)
	{
		this(executors, Runtime.getRuntime().availableProcessors());
	}

	/**
	 * Creates a new default SmppClient. Window monitoring and automatic expiration of requests will be disabled with no monitorExecutors.
	 * 
	 * @param executor
	 *            The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is recommended. The max threads will never grow more than expectedSessions if NIO sockets
	 *            are used.
	 * @param expectedSessions
	 *            The max number of concurrent sessions expected to be active at any time. This number controls the max number of worker threads that the underlying Netty library will use. If
	 *            processing occurs in a sessionHandler (a blocking op), be <b>VERY</b> careful setting this to the correct number of concurrent sessions you expect.
	 */
	public SmppClient(ExecutorService executors, int expectedSessions)
	{
	 ...
	 *            The scheduled executor that all sessions will share to monitor themselves and expire requests. If null monitoring will be disabled.
	 */
	public SmppClient(ExecutorService executors, int expectedSessions, ScheduledExecutorService monitorExecutor)
	{
	...
	@SuppressWarnings("rawtypes")
	protected void doBind(SmppSession session, SmppSessionConfiguration config, SmppSessionHandler sessionHandler)
			throws SmppTimeoutException, SmppChannelException, SmppBindException, UnrecoverablePduException, InterruptedException
	{
		...
		try
		{
			session.bind(bindRequest, config.getBindTimeout());
		}
	...
	protected SmppSession doOpen(SmppSessionConfiguration config, SmppSessionHandler sessionHandler) throws SmppTimeoutException, SmppChannelException, InterruptedException
	{
	...
	protected SmppSession createSession(Channel channel, SmppSessionConfiguration config, SmppSessionHandler sessionHandler) throws SmppTimeoutException, SmppChannelException, InterruptedException
	{
		SmppSession session = new SmppSession(SmppSession.Type.CLIENT, config, channel, sessionHandler, monitorExecutor);
		...
		{
			logger.warn("Smpp Connector - Session configuration did not have a name set - skipping threadRenamer in pipeline");
		}

		// create the logging handler (for bytes sent/received on wire)
		SmppSessionLogger loggingHandler = new SmppSessionLogger(SmppSession.class.getCanonicalName(), config.getLoggingOptions());
	...
	@Override
	public com.cloudhopper.smpp.SmppSession bind(SmppSessionConfiguration arg0, com.cloudhopper.smpp.SmppSessionHandler arg1)
			throws SmppTimeoutException, SmppChannelException, SmppBindException, UnrecoverablePduException, InterruptedException
	{
		return null;
	}
