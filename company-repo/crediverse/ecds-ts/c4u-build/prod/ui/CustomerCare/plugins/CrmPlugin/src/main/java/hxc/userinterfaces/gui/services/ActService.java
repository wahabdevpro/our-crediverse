...
public class ActService implements ISubscriptionService
{
	...
	{
		GetCreditTransfersResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);
			GetCreditTransfersRequest req = new GetCreditTransfersRequest();
			...
			req.setMode(test ? RequestModes.TEST_ONLY : RequestModes.NORMAL);
			req.setActiveOnly(activeOnly);
			req.setSubscriberNumber(serviceProvider.generateMSISDNNumber(msisdn));
			req.setServiceID(serviceId);
			req.setVariantID(variantId);
			req.setMemberNumber(serviceProvider.generateMSISDNNumber(memberMsisdn));

			if (transferMode != null)
				req.setTransferMode(transferMode);

			result = port.getCreditTransfers(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		ChangeCreditTransferResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);

			ChangeCreditTransferRequest req = new ChangeCreditTransferRequest();
			...
			req.setDonorNumber(serviceProvider.generateMSISDNNumber(msisdn));
			req.setRecipientNumber(serviceProvider.generateMSISDNNumber(memberMsisdn));

			result = port.changeCreditTransfer(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		AddCreditTransferResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);

			AddCreditTransferRequest req = new AddCreditTransferRequest();
			...
			if (transferLimit != null)
				req.setTransferLimit(transferLimit);

			if (transferMode != null)
				req.setTransferMode(transferMode);

			result = port.addCreditTransfer(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		RemoveCreditTransfersResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);

			RemoveCreditTransfersRequest req = new RemoveCreditTransfersRequest();
			...
			result = port.removeCreditTransfers(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		SuspendCreditTransferResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);

			SuspendCreditTransferRequest req = new SuspendCreditTransferRequest();
			...
			result = port.suspendCreditTransfer(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		ResumeCreditTransferResponse result = null;
		try
		{
			IHxC port = serviceProvider.getSOAPServicePort(user);

			ResumeCreditTransferRequest req = new ResumeCreditTransferRequest();
			...
			result = port.resumeCreditTransfer(req);
		}
		catch (Exception e)
		{
			e.printStackTrace();
