...
// //////////////////////////////////////////////////////////////////////////////////////
//
// This class is partially based on Entities.java
// which is part of Apache Commons Lang 3.1 library
//
// /////////////////////////////////
...
public class StringUtils
{
	...
	static final String[][] HTML40_ARRAY = {
			...
			// <!-- ISO 10646 does not say that lArr is the same as the 'is implied by' arrowbut also does not have any other character for that function. So ? lArr canbe used for 'is implied by' as
			// ISOtech suggests -->
			...
			// <!-- note that nsup, 'not a superset of, U+2283' is not covered by the Symbol font encoding and is not included. Should it be, for symmetry?It is in ISOamsn --> <!ENTITY nsub", "8836"},
	...
	static final String[][] ESCAPE_CHARACTERS = { { "newline", "" + (int) '\n' }, { "tab", "" + (int) '\t' }, { "return", "" + (int) '\r' }, { "", "" + (int) '\f' }, { "", "" + (int) '\b' }, };
	 ...
	 * </p>
	 */
	public static final StringUtils XML;
	 ...
	 * </p>
	 */
	public static final StringUtils HTML32;
	 ...
	 * </p>
	 */
	public static final StringUtils HTML40;

	/**
	 * <p>
	 * The set of entities supported by Java Escape Characters
	 * </p>
	 */
	public static final StringUtils ESCAPE;
	...
	static
	{
		HTML40 = new StringUtils();
		fillWithHtml40Entities(HTML40);
	}

	static
	{
		ESCAPE = new StringUtils();
		ESCAPE.addEntities(ESCAPE_CHARACTERS);
	}

	static void fillWithHtml40Entities(StringUtils entities)
	{
	...
	static abstract class MapIntMap implements StringUtils.EntityMap
	{
	...
	// package scoped for testing
	EntityMap map = new StringUtils.LookupEntityMap();
	...
	public static String toLatinNumerals(String text)
	{
		// Defensive
	...
	private static final String zeroes = "0000000000000000000000000000000000000000000000";

	public static String formatScaled(long quantity, long numerator, long denominator)
	{
		int decimalDigits = getDecimalDigits(numerator, denominator);
		int multiplier = 1;
		...
		String digits = Long.toString((quantity * multiplier * denominator + (numerator / 2)) / numerator);

		if (decimalDigits > 0)
		{
			if (digits.length() < decimalDigits + 1)
				digits = zeroes.substring(0, decimalDigits - digits.length() + 1) + digits;
			digits = digits.substring(0, digits.length() - decimalDigits) + "." + digits.substring(digits.length() - decimalDigits, digits.length());
		}

		return digits;
	}

	public static long parseScaled(String quantity, long numerator, long denominator)
	{
		int multiplier = 1;
		int point = quantity.indexOf('.');
		if (point >= 0)
		{
			int decimalDigits = quantity.length() - point - 1;
			for (int index = 0; index < decimalDigits; index++)
			{
				multiplier *= 10;
			}
		...
		long divisor = multiplier * denominator;
		return (value * numerator + divisor / 2) / divisor;
	}

	private static int getDecimalDigits(long numerator, long denominator)
	{
		long factor = (denominator + numerator - 1) / numerator;
		if (factor <= 0)
			return 0;
		int result = 0;
