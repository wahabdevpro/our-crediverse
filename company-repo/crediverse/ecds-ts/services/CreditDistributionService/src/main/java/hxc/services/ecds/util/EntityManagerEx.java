...
import hxc.utils.general.StackTraceInfo;

public class EntityManagerEx implements EntityManager, AutoCloseable
{
	private final EntityManager em;
	private final PooledDataSource pooledDataSource;
	final static Logger logger = LoggerFactory.getLogger(EntityManagerEx.class);

	private EntityManagerEx(EntityManager em, PooledDataSource pooledDataSource)
	{
		// DO NOT MAKE THIS PUBLIC WITHOUT CHANGING TRACKING/LOGGING
		this.em = em;
		this.pooledDataSource = pooledDataSource;
	}

	public static EntityManagerEx create(EntityManagerFactory factory, PooledDataSource pooledDataSource)
	{
		EntityManager em = null;
		EntityManagerEx result = null;
		try {
			em = factory.createEntityManager();
			result = new EntityManagerEx(factory.createEntityManager(), pooledDataSource);
			logger.trace("({})<init>: em = {}, pooledDataSource.connectionSummary = ({}) StackTraceInfo.here().summaryAll() = {}", result.hashCode(), em, PooledDataSourceUtils.connectionSummariser( pooledDataSource ), new StackTraceInfo.Summariser(StackTraceInfo.here(1), null, 0, 6, true));
			//DbUtils.makeRepeatableRead(result);
			return result;
		}
		catch( Throwable throwable ) {
			logger.error("create: caught", throwable);
			if (em != null) em.close();
			throw throwable;
		}
	}
	...
	{
		if (em.isOpen()) {
			logger.trace("({})close: em = {}", this.hashCode(), em);
			em.close();
		}
	}
