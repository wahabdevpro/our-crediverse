...
public class OlapCleaner implements Runnable, AutoCloseable, IConfigurationChange
{
	final static Logger logger = LoggerFactory.getLogger(OlapCleaner.class);
	...
	public OlapCleaner(IServiceBus esb, ICreditDistribution context, IService service, CompanyInfo companyInfo) throws RuntimeException
	{
		this.esb = esb;
	...
	public void runActual() throws Exception
	{
		...
		try (EntityManagerEx apEm = this.context.getApEntityManager())
		{
			...
			logger.info("deleting all entires older than {} days", this.retentionDays);
			int deleted = -1;
			try (RequiresTransaction scope = new RequiresTransaction(apEm))
			{
				deleted = OlapTransaction.deleteOlderThan(apEm, this.retentionDays);
				scope.commit();
			}
			logger.info("deleted {} entries", deleted);
		}
		finally
		{
			synchronized (this.runCounterMonitor)
	...
	public void onConfigurationChanged(IConfiguration configuration)
	{
		...
		this.retentionDays = config.getOlapTransactionRetentionDays();
		this.timeOfDay = config.getOlapTransactionCleanupTimeOfDay();
