...
import hxc.ecds.protocol.rest.PartialReversalResponse;
...
@Path("/transactions")
public class PartialReverse extends Transactions<hxc.ecds.protocol.rest.PartialReversalRequest, hxc.ecds.protocol.rest.PartialReversalResponse>
{
	final static Logger logger = LoggerFactory.getLogger(PartialReverse.class);
	...
	@POST
	@Path("/partially_reverse")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public hxc.ecds.protocol.rest.PartialReversalResponse execute(hxc.ecds.protocol.rest.PartialReversalRequestWithCoAuth request)
	{
		return super.execute(request);
	}

	@POST
	@Path("/partially_reverse_without_co_auth")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	@Override
	public hxc.ecds.protocol.rest.PartialReversalResponse execute(hxc.ecds.protocol.rest.PartialReversalRequest request) {
		return super.execute(request);
	}
	...
	{
		return Transaction.TYPE_REVERSE_PARTIALLY;
	}

	@Override
	protected void validate(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state) throws RuleCheckException
	{
		// Record Request Information
		Transaction transaction = state.getTransaction();
		PartialReversalRequest request = state.getRequest();
		...
		if (coAuthEnabled) {
			if (request instanceof PartialReversalRequestWithCoAuth) {
				validateCoAuth(em, state, (PartialReversalRequestWithCoAuth) request, session);
			} else {
				logger.error("Co-authorization enabled but the request is not PartialReversalRequestWithCoAuth.");
		...
		if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0 || amount.compareTo(original.getAmount()) > 0)
			throw new RuleCheckException(TransactionsConfig.ERR_INVALID_AMOUNT, "ammount", "Invalid Amount specified", request.getTransactionNumber());
	...
	private void validateCoAuth(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state, PartialReversalRequestWithCoAuth request, Session session) throws RuleCheckException {
	...
	@Override
	protected void execute(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state) throws RuleCheckException {
	...
	private void reverseTransfer(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state) throws RuleCheckException {
		...
		BigDecimal amountToReverse  = state.getRequest().getAmount();

		BigDecimal ratioToReverse = amountToReverse.divide(original.getAmount(),8,RoundingMode.CEILING);
		...
		BigDecimal buyerTradeBonusEarnedToReverse = originalBuyerTradeBonusEarned.multiply(ratioToReverse);
		...
		BigDecimal buyerBonusProvisionToReverse = 
			(buyerTradeBonusProvisionAfter.subtract(buyerTradeBonusProvisionBefore))
			.multiply(ratioToReverse).negate();
		...
		try (RequiresTransaction trans = new RequiresTransaction(em)) {
			...
			transaction.setBuyerTradeBonusAmount(
					buyerTradeBonusEarnedToReverse.negate()
					.setScale(currencyDecimalDigits, RoundingMode.CEILING));
			transaction.setBuyerTradeBonusProvision(
					buyerBonusProvisionToReverse
					.setScale(currencyDecimalDigits, RoundingMode.CEILING));

			transaction.setBuyerTradeBonusPercentage(
					originalBuyerTradeBonusPercentage
					.setScale(currencyDecimalDigits, RoundingMode.CEILING));
		...
		try {
			...
			List<String> externalDataList = 
				this.expandExternalDataList(
						Arrays.asList(config.getUbadExternalData1(), config.getUbadExternalData2()), 
						config.listExternalDataFields(), 
						state);
			...
			try {
				...
				state.exitWith(
						TransactionsConfig.ERR_ACC_NOT_FOUND, 
						"Could not find dedicated account with id " + ex.getDedicatedAccountId());
				try (RequiresTransaction trans = new RequiresTransaction(em)) {
			...
			try (RequiresTransaction trans = new RequiresTransaction(em)) {
				aAccount.reverse(original.getStartTime(), 
						amountToReverse
						.setScale(currencyDecimalDigits, RoundingMode.CEILING), 
						buyerTradeBonusEarnedToReverse
						.setScale(currencyDecimalDigits,RoundingMode.CEILING), 
						buyerBonusProvisionToReverse
						.setScale(currencyDecimalDigits, RoundingMode.CEILING), false);
				bAccount.adjust(
						amountToReverse
						.add(buyerTradeBonusEarnedToReverse)
						.negate()
						.setScale(currencyDecimalDigits, RoundingMode.CEILING), 
						buyerBonusProvisionToReverse
						.setScale(currencyDecimalDigits, RoundingMode.CEILING), false);
	...
	private void reverseSale(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state) throws RuleCheckException {
		final Transaction transaction = state.getTransaction();
		Transaction original = transaction.getOriginalTransaction();
		BigDecimal partialReverseAmount = state.getRequest().getAmount();
		transaction.setAmount(partialReverseAmount.negate());
		...
		BigDecimal transactionAmountToReverse = partialReverseAmount.add(BigDecimal.ZERO);
		// Execute the UBD to Debit Subscriber
		try {
			IAirConnector air = context.getAirConnector();
			Long maAmountToReverse = air.toLongAmount(partialReverseAmount);
	...
	@Override
	protected void conclude(EntityManager em, TransactionState<PartialReversalRequest, PartialReversalResponse> state)
	{
		ReversalsConfig config = state.getConfig(em, ReversalsConfig.class);
	...
	@Override
	public String expandField(String englishName, Locale locale, TransactionState<PartialReversalRequest, PartialReversalResponse> state)
	{
		Transaction transaction = state.getTransaction();
		switch (englishName)
		{
	...
	@Override
	public String expandExternalDataField(String englishName, TransactionState<PartialReversalRequest, PartialReversalResponse> state)
	{
		Transaction transaction = state.getTransaction();
		switch (englishName)
		{
