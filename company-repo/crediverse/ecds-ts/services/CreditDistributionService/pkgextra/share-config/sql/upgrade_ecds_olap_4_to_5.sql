...
BEGIN
	DECLARE old_version int(11) DEFAULT 4;
	DECLARE new_version int(11) DEFAULT 5;
		...
		START TRANSACTION;
			...
			IF NOT EXISTS (SELECT null FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ap_transact' AND COLUMN_NAME = '') then
				...
				ALTER TABLE  `ap_transact` 
					ADD  `follow_up` TINYINT NULL DEFAULT NULL ,
					ADD  `rolled_back` TINYINT NULL DEFAULT NULL ;
