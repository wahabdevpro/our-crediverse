...
BEGIN
	...
	DECLARE old_version int(11) DEFAULT 38;
	DECLARE new_version int(11) DEFAULT 39;
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
			...
			/* Add column email to table es_webuser */
      if not exists (select null 
            ...
            alter table `es_webuser` add column `email` varchar(50) default null;

            select "Added column email to table es_webuser" as "INFO";
            
      end if;   
      
      /* Add column email to table eb_stage */
      if not exists (select null 
                     from information_schema.COLUMNS 
                     where TABLE_SCHEMA = DATABASE()
                     and COLUMN_NAME = 'email' 
            ...
            alter table `eb_stage` add column `email` varchar(50) default null;

            select "Added column email to table eb_stage" as "INFO";
