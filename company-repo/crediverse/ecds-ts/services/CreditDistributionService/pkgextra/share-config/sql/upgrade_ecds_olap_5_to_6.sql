...
BEGIN
	DECLARE old_version int(11) DEFAULT 5;
	DECLARE new_version int(11) DEFAULT 6;
	...
	IF current_version_string != new_version THEN
		...
		START TRANSACTION;
			...
			IF NOT EXISTS (SELECT null FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ap_transact' AND COLUMN_NAME = 'related_id') then

				ALTER TABLE  `ap_transact` 
					ADD  `related_id` INT NULL DEFAULT NULL ;
