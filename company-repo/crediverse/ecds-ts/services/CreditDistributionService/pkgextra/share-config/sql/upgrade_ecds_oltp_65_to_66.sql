...
BEGIN
	...
	DECLARE old_version int(11) default 65;
	DECLARE new_version int(11) default 66;
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
	        ...
	        SELECT "Checking er_report_schedule.email_to_agent" AS "INFO";
	        IF NOT EXISTS (SELECT null from information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE()
	            AND TABLE_NAME = 'er_report_schedule' AND COLUMN_NAME = 'email_to_agent') THEN
	            SELECT "Adding er_report_schedule.email_to_agent" AS "INFO";
				ALTER TABLE  `er_report_schedule` ADD  `email_to_agent` TINYINT NOT NULL DEFAULT  '0' AFTER  `delivery_channels` ;
	            SELECT "Added er_report_schedule.email_to_agent" AS "INFO";
	        END IF;
