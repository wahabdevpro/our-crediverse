...
BEGIN
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
			...
			IF EXISTS (SELECT 'exists' FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND `TABLE_NAME` = 'es_audit' AND `COLUMN_NAME` = 'id' AND LOWER(DATA_TYPE) != 'bigint' AND LOWER(SUBSTRING(COLUMN_TYPE, 1, 6)) != 'bigint')
			THEN
				SELECT CONCAT("Changing es_audit.id to BIGINT") AS "INFO";
				ALTER TABLE es_audit
					MODIFY COLUMN id BIGINT NOT NULL AUTO_INCREMENT;
				SELECT CONCAT("Completed changing es_audit.id to BIGINT") AS "INFO";
			ELSE
				SELECT CONCAT("Skipping es_audit.id BIGINT column change.") AS "INFO";
			END IF;
