...
BEGIN
	DECLARE old_version int(11) DEFAULT 80;
	DECLARE new_version int(11) DEFAULT 81;
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
			...
			/* MEAT: START */
			/* If it exists, drop original version 81 column */
			SELECT count(*) INTO problem_records FROM ec_transact a, ec_transact rel where a.type in ('PA', 'FR') and a.reversed_id = rel.id and rel.type in ('SL','SB','ST') and a.amount > 0;
			IF problem_records > 0 THEN
				UPDATE ec_transact u JOIN ec_transact rel ON u.reversed_id = rel.id
