...
BEGIN
	...
	DECLARE old_version int(11) default 54;
	DECLARE new_version int(11) default 55;
	...
	if current_version != new_version then
      ...
      -- Add column confirm_ussd to table ea_agent      
			if not exists (select null 
                     from information_schema.COLUMNS 
                     where TABLE_SCHEMA = DATABASE()
                     and COLUMN_NAME = 'confirm_ussd' 
                     and TABLE_NAME = 'ea_agent') then

				alter table ea_agent add column `confirm_ussd` bit(1) default 1;
        select "Added column confirm_ussd to table ea_agent" as "INFO";        
