...
BEGIN
	DECLARE old_version int(11) DEFAULT 96;	/* FIXME replace XX with the version this script is upgrading from, e.g. 12 */
	DECLARE new_version int(11) DEFAULT 97; /* FIXME replace YY with the version this script is upgrading to, e.g. 13  */
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
			...
			UPDATE `hxc`.`ea_agent` SET send_daily_bundle_commission_report = 0 WHERE state != 'A';
