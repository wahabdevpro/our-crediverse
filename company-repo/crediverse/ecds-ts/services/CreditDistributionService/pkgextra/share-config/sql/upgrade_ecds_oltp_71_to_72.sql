...
begin
	...
	declare old_version int(11) default 71;
	declare new_version int(11) default 72;
	...
	if current_version != new_version then
        ...
        -- Add index ix_ec_transact_aowner to table ec_transact
        if not exists (select null 
            ...
            alter table `ec_transact`                  
				add index `ix_ec_transact_aowner` (`a_owner` asc);

            select "Added index ix_ec_transact_aowner to table ec_transact" as `INFO`;

        end if;
        
        -- Add index ix_ec_transact_bowner to table ec_transact
        if not exists (select null 
                     from information_schema.statistics 
                     where TABLE_SCHEMA = database()
            ...
            alter table `ec_transact`                  
				add index `ix_ec_transact_bowner` (`b_owner` asc);

            select "Added index ix_ec_transact_bowner to table ec_transact" as `INFO`;
