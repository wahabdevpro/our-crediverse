...
BEGIN
	DECLARE old_version int(11) DEFAULT 85;
	DECLARE new_version int(11) DEFAULT 86;
	...
	IF current_version != old_version THEN
		...
		START TRANSACTION;
            ...
            ALTER TABLE `hxc`.`ea_agent`
            DROP INDEX `es_agent_msisdn`;

            ALTER TABLE `hxc`.`ea_agent`
            ADD CONSTRAINT es_agent_msisdn
            UNIQUE (`msisdn`);

            ALTER TABLE `es_webuser`
            MODIFY `service_user` int(11) DEFAULT NULL;
