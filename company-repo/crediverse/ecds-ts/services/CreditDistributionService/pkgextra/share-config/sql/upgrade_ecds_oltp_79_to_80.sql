...
BEGIN
	DECLARE old_version int(11) DEFAULT 79;
	DECLARE new_version int(11) DEFAULT 80;
	...
	IF current_version != new_version THEN
		...
		START TRANSACTION;
			...
			IF NOT EXISTS(SELECT 'exists' FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND `TABLE_NAME` = 'es_audit' AND `COLUMN_NAME` = 'reason_code')
				AND NOT EXISTS(SELECT 'exists' FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND `TABLE_NAME` = 'es_audit' AND `COLUMN_NAME` = 'reason_attributes')
			THEN
				ALTER TABLE es_audit 
					ADD COLUMN `reason_code` VARCHAR(64) DEFAULT NULL,
					ADD COLUMN `reason_attributes` TEXT DEFAULT NULL;
			ELSE
				SELECT "Columns 'reason_code' and 'reason_attributes' already exist." AS "INFO";
			END IF;
