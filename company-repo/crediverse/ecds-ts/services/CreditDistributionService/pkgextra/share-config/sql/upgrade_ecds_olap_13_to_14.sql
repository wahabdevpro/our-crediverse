...
BEGIN
	DECLARE old_version int(11) DEFAULT 13;
	DECLARE new_version int(11) DEFAULT 14;
	...
	IF current_version_string != new_version THEN
		...
		START TRANSACTION;
			...
			IF NOT EXISTS(SELECT 'exists' FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND `TABLE_NAME` = 'ap_transact' AND `COLUMN_NAME` = 'b_transfer_bonus_amount')
				AND NOT EXISTS(SELECT 'exists' FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND `TABLE_NAME` = 'ap_transact' AND `COLUMN_NAME` = 'b_transfer_bonus_profile')
			THEN
