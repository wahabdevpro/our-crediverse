...
BEGIN
	DECLARE old_version int(11) DEFAULT 9;
	DECLARE new_version int(11) DEFAULT 10;
	...
	IF current_version_string != new_version THEN
		...
		START TRANSACTION;
			...
			IF NOT EXISTS (SELECT "1" FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ap_transact' AND COLUMN_NAME = 'a_owner_id') THEN
				ALTER TABLE  `ap_transact` 
					ADD  `a_owner_id` INT NULL DEFAULT NULL AFTER  `a_owner_name` ;
			END IF;
			
			IF NOT EXISTS (SELECT "1" FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'ap_transact' AND COLUMN_NAME = 'b_owner_id') THEN
				ALTER TABLE  `ap_transact` 
					ADD  `b_owner_id` INT NULL DEFAULT NULL AFTER  `b_owner_name` ;
			END IF;
