...
BEGIN
	...
	DECLARE old_version int(11) DEFAULT 48;
	DECLARE new_version int(11) DEFAULT 49;
	...
	if current_version != new_version then
      ...
      -- Drop Index ec_transact_revid on Table ec_transact if it exists
			if exists (select null 
                from information_schema.statistics 
                where TABLE_SCHEMA = DATABASE() 
                and table_name = 'ec_transact' 
                and index_name = 'ec_transact_revid') then
                
				alter table `ec_transact` drop index `ec_transact_revid` ;
       	select "Dropped index ec_transact_revid on table ec_transact" as "INFO";
