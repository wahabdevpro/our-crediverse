...
public class RestRequestUtil
{
	...
	//Horrible: TDR specific
	static public void includeQuery(UriComponentsBuilder uri, boolean includeQuery)
	{
		uri.replaceQueryParam(INCLUDEQUERY, String.valueOf(includeQuery?1:0));
	}
	
	static public void includeFetchSubscriberState(UriComponentsBuilder uri, boolean fetchSubscriberState) {
		uri.replaceQueryParam(FETCH_SUBSCRIBER_STATE, String.valueOf(fetchSubscriberState) );
	} 
	
	static public void addCustomParameters(UriComponentsBuilder uri, IQueryStringParameters params)
	{
		if (params != null) 
			params.addParameters(uri);
	}
}
