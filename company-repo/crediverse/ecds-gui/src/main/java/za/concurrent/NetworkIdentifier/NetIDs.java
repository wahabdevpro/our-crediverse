package za.concurrent.NetworkIdentifier;
...
public class NetIDs implements Runnable, IDsReceiver {
		...
		registry = CacheBuilder.newBuilder()
				...
				.build( new CacheLoader<String, NetID>()
				{
					...
					public NetID load( String key ) throws Exception {
						logger.trace( "creating missing print..." );
						NetID np = createMissingPrint( key );
						logger.trace( "created missing print" + np );
						return np;
					}
				}
						);
	...
	{
		// isRunning = true; /// UNUSED ?
		logger.debug("processing missing id entries...");
		idsPool.submit(this);
	}
	...
	public void run( )
	{
		logger.trace("processing list of missing");
		synchronized( this ){
			...
			while ( l.hasNext() ){
				...
				NetID np = l.next();
				logger.debug("processing missing item {}", np.getIpAdress());
				try {
	...
	{
		logger.trace( "recording Host info, ip: " + ipAddr + " host: " + hostname );

		// TODO: can we avoid these copies? Guava caches must have a solution to this
		if( !hostname.equals("localhost") && !hostname.equals(ipAddr) )
		{
			...
			try {
				...
				logger.trace( "Updated netprint record with host info: " + np );
				registry.put( ipAddr, np );			
			} catch (ExecutionException e) {
				logger.warn( "Execution exception while accessing prints cache " + 
	...
	{
		logger.trace( "recording MAC info, ip: " + ipAddr + " mac: " + macAddress );
		if( ! macAddress.equals( MacAddress.ETHER_BROADCAST_ADDRESS.toString() ) )
		{
			...
			try {
				...
				np.setLastUpdated( new Date() );
				logger.trace( "Updated netprint record with mac address: " + np );
				registry.put( ipAddr, np );
			} catch (ExecutionException e) {
				logger.warn( "Execution exception while accessing prints cache " + 
						e.getLocalizedMessage() + "\n" + e.getStackTrace().toString() );
