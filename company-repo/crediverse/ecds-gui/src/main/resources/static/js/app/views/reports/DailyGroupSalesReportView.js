define( ['jquery', 'underscore', 'App', 'backbone', 'marionette', "handlebars", 'collections/TierCollection', 
         'utils/HandlebarHelpers', 'models/ReportModel', 'views/reports/DailyGroupSalesReportTableView', 'utils/CommonUtils', 'jquery.maskedinput', 'datatables'],
    function($, _, App, BackBone, Marionette, Handlebars, TierCollection, HBHelper, ReportModel, DailyGroupSalesReportTableView, CommonUtils, maskedinput) {

		var i18ntxt = App.i18ntxt.reports.dailyGroupSales;
        var DailyGroupSalesReportView =  DailyGroupSalesReportTableView.extend( {
        	tagName: 'div',
        	...
        	attributes: {
  		  	...
  		  	template: "Reports#dailygroupsales",
  		  	url: 'api/reports/dailygroupsales',
  		  	error: null,
  		  	model: null,
			sorting: null,
			id: null,
			...
			events: {
  		  	...
  		  	breadcrumb: function() {
  		  		...
  		  		return {
  		  			heading: txt.dailyGroupSales.heading,
  		  			defaultHome: false,
  		  			breadcrumb: [{
						text: txt.report,
						iclass: "fa fa-file-text"
					},{
  		  				text: txt.dailyGroupSales.headingBC,
						href: '#dailyGroupSalesReportList',
  		  			}]
  		  		}
  		  	},
			...
			loadModel: function () {
				...
				if (self.id != null) {
	            	self.model = new ReportModel({id: self.id, report:'dailygroupsales'});
	            	self.model.fetch({
	            		success: function(ev){
							if (self.model.attributes.parameters) {
								...
								if (params.filter && params.filter.items.length) {
									for (var i = 0; i < params.filter.items.length; ++i) {
										...
										switch(item.field)
										{
										case "TIER_NAME": self.model.set("a_TierName", item.value); break;
										case "GROUP_NAME": self.model.set("a_GroupName", item.value); break;
	            	...
	            	self.model = new ReportModel({id: null, report:'dailygroupsales'});
					self.loadTiers();
				}	
            ...
            configureSearchForm: function() {
            	var self = this;
				var baseUrl = '#dailyGroupSalesReport';
			...
			cancelReport: function(ev) {
				App.appRouter.navigate('#dailyGroupSalesReportList', {trigger: true, replace: true});
			...
			saveReport: function(ev) {
				...
				$('#transactionType').rules('add', {required: true});
            	if (self.$( "#searchform" ).valid()) {
					/**
        ...
        return DailyGroupSalesReportView;
    });
