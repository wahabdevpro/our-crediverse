define( ['jquery', 'underscore', 'App', 'views/reports/DailySalesByAreaReportTableView', 'utils/CommonUtils', 'models/ReportModel', 'models/FeatureBarModel'],
    function($, _, App, DailySalesByAreaReportTableView, CommonUtils, ReportModel, FeatureBarModel ) {

		var i18ntxt = App.i18ntxt.reports.dailySalesByArea;
        var DailySalesByAreaReportView =  DailySalesByAreaReportTableView.extend( {
        	tagName: 'div',
        	attributes: {
        		//class: "row",
        		id: 'dailysalesbyarea'
        	},
  		  	template: "ReportsByArea#dailysalesbyarea",
  		  	url: 'api/reports/area/daily_sales',
  		  	error: null,
  		  	model: null,
			sorting: [],
			id: null,
			...
			viewByAreaReports_FeatureName: 'viewReportsByAreaFeature',
			...
			events: {
  		  	...
  		  	breadcrumb: function() {
  		  		...
  		  		return {
  		  			heading: txt.dailySalesByArea.heading,
  		  			defaultHome: false,
  		  			breadcrumb: [{
						text: txt.report,
						iclass: "fa fa-file-text"
					},{
  		  				text: txt.dailySalesByArea.headingBC,
						href: '#dailySalesByAreaReportList',
  		  			}]
  		  		}
  		  	},
            ...
            initialize: function (options) {
				...
				this.featureBar.setFeature(this.viewByAreaReports_FeatureName);
			...
			loadModel: function () {
				...
				if (self.id != null) {
	            	self.model = new ReportModel({id: self.id, report:'area/daily_sales'});
	            	self.model.fetch({
	            		success: function(ev){
							...
							self.featureBar.fetch().then((isEnabled) => {
								self.model.set(self.viewByAreaReports_FeatureName, isEnabled ? "enabled" : "disabled");
								self.render();
							});
						}
					});
	            	...
	            	self.model = new ReportModel({id: null, report:'dailysalesbyarea'});
					self.featureBar.fetch().then((isEnabled) => {
						self.model.set(self.viewByAreaReports_FeatureName, isEnabled ? "enabled" : "disabled");
						self.render();
					});
			...
			resetModel: function () {
            	var self = this;
				if (self.id != null) {
	            	self.model = new ReportModel({id: self.id, report:'dailysalesbyarea'});
            ...
            configureSearchForm: function() {
            	var self = this;
				var baseUrl = '#dailySalesByAreaReport';
            ...
            getFormDataAsync: function() {
				...
				return self.featureBar.fetch().then((isEnabled) => {
					/**
					 * Always set to 'CURRENT_DAY' if feature is disabled.
					 */
					criteria.period = isEnabled ? criteria.period : 'CURRENT_DAY';
			...
			exportReport: function(ev) {
				...
				if (self.$( "#searchform" ).valid()) {
					var table = this.$('.dailysalesbyareatable');
					var pos = self.url.indexOf('?')
					/**
					 * FIXME --- not enabled ... make sure that it works
					 */
					return self.getFormDataAsync().then((formData) => {
			...
			resetReport: function(ev) {
				...
				self.$("#a_TierName,#a_GroupName").val(null).trigger("change");
				self.currentFilter.data = "";
				self.configureSearchForm();
			},

			cancelReport: function(ev) {
				App.appRouter.navigate('#dailySalesByAreaReportList', {trigger: true, replace: true});
			},
			...
			saveReport: function(ev) {
				...
				if (self.$("#searchform" ).valid()) {
					return self.getFormDataAsync().then((urlEncodedFormData) => {
						/**
						...
						var url = self.url+(self.id != null ? '/'+self.id : '')+'?'+urlEncodedFormData + dtData;
        ...
        return DailySalesByAreaReportView;
    });
