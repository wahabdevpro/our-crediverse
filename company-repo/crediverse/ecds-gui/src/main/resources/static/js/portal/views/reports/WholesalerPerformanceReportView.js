define( ['jquery', 'underscore', 'App', 'backbone', 'marionette', "handlebars", 'collections/TierCollection', 
    ...
    function($, _, App, BackBone, Marionette, Handlebars, TierCollection, HBHelper, ReportModel, WholesalerPerformanceReportTableView, CommonUtils, maskedinput) {
        ...
        var WholesalerPerformanceReportView =  WholesalerPerformanceReportTableView.extend( {
        	...
        	attributes: {
  		  	...
  		  	url: 'papi/reports/wholesalerperformance',
  		  	error: null,
  		  	model: null,
			sorting: null,
			id: null,
			...
			loadModel: function () {
				...
				if (self.id != null) {
					...
					self.model.set("disposition", "outbound");
	            	self.model.fetch({
	            		success: function(ev){
							if (self.model.attributes.parameters) {
								...
								if (params.filter && params.filter.items.length) {
									for (var i = 0; i < params.filter.items.length; ++i) {
										...
										case "DISPOSITION": self.model.set("disposition", item.value); break;
										case "TOTAL_AMOUNT": 
            ...
            onRender: function () {
				...
				if(params && params.sort) {
					for(var i = 0; i < params.sort.items.length; ++i) {
						...
						case 'B_ACCOUNT_NUMBER': colIx = 14; break;
						case 'B_MOBILE_NUMBER': colIx = 15; break;
						case 'B_IMSI': colIx = 16; break;
						case 'B_NAME': colIx = 17; break;
						case 'B_TIER_NAME': colIx = 18; break;
						case 'B_GROUP_NAME': colIx = 19; break;
						case 'B_SERVICE_CLASS_NAME': colIx = 20; break;
						case 'B_OWNER_IMSI': colIx = 21; break;
						case 'B_OWNER_MOBILE_NUMBER': colIx = 22; break;
						case 'B_OWNER_NAME': colIx = 23; break;
						case 'TOTAL_AMOUNT': colIx = 24; break;
						case 'TOTAL_BONUS': colIx = 25; break;
						case 'TRANSACTION_COUNT': colIx = 26; break;
						}
						var colDir = 'asc';
				...
				self.$('#disposition').val(self.model.get("disposition"));
			...
			resetReport: function(ev) {
				...
				self.$("#b_TierName,#b_GroupName,#b_ServiceClassName").val(null).trigger("change");
