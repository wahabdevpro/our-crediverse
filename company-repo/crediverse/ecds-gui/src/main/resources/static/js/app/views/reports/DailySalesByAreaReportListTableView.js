define( ['jquery', 'underscore', 'App', 'backbone', 'marionette', 'utils/CommonUtils', 'views/reports/ReportListTableView'],
    function($, _, App, BackBone, Marionette, CommonUtils, ReportListTableView) {
        ...
        var DailySalesByAreaReportListTableView = ReportListTableView.extend( {
        	currentFilter: {}, // Used to keep track of filter settings for use by export.
        	i18ntxt: App.i18ntxt.reports.dailySalesByAreaList,
            ...
            renderTable: function(options) {
            	...
            	var table = this.$('.dailysalesbyarealisttable');
                      ...
                      "columns": [
                    	   {
							   ...
							   render: function(data, type, row, meta) {
								   ...
								   //return data;
								   // Make it clickable so it can be Viewed/Edited
								    var url = '#dailySalesByAreaReport/' + row.id + '/asf!on' + params;
                    	   ...
                    	   },
                    	   {
							   ...
							   render: function(data, type, row, meta) {
								   ...
								   return self.formatSchedule(data, undefined, { excludeHourly: true });
							   }
                    	   },
                    	   {
                    		   data: null,
							   ...
							   render: function(data, type, row, meta) {
                   	            	...
                   	            	if (App.hasPermission("Reports", "Update")) {
                   	            		buttons.push('<button type="button" class="btn btn-primary btn-xs scheduleReportButton" data-exclude-hourly="true">'+App.i18ntxt.global.scheduleBtn+'</button>');
                   	            		buttons.push("<a href='#dailySalesByAreaReport/"+row.id+"/edit' class='btn btn-primary routerlink btn-xs'>"+App.i18ntxt.global.editBtn+"</a>");
                   	            	}
        ...
        return DailySalesByAreaReportListTableView;
    });
