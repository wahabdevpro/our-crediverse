define( ['jquery', 'App', 'underscore', 'marionette', 'utils/CommonUtils',
         'views/config/GenericConfigDialog',
         'file-upload', 'jquery-sortable'],
    function($, App, _, Marionette, CommonUtils,
    		GenericConfigDialog) {

        var UssdCommandView = GenericConfigDialog.extend( {
        	template: 'UssdMenuConfiguration#ussdMenuCommand',
        	
        	tagName: 'div',
        	attributes: {
        		class: "modal-content",
        		id: "ussdMenuItem"
        	},
        	
        	
        	i18ntxt: App.i18ntxt.ussdmenu,
        	ui: {
        		itemSaveButton: '.itemSaveButton',
        	    ussdmenucommand:	'.ussdmenucommand'
        	},
        	
        	// DOM Events
            events: {
            	"click @ui.itemSaveButton": 'saveItem',
            	"keypress @ui.ussdmenucommand": 'validateCommand'
            },
            
            validateCommand: function(ev) {
            	var keyValue = ev.which;
            	//console.log(ev.which);
            	if (keyValue !== 35 && // # key
            		keyValue !== 42 && // * key
            		keyValue !== 8 && // backspace key
            		keyValue !== 13 && // enter key
            		(keyValue < 48 || keyValue > 57) &&
            		ev.keyCode !== 9 // tab key
            		) {
            		return false;
            	}
            	if (keyValue === 13) {
            		this.saveItem(ev);
            		return false;
            	}
            	return true;
            },
            
            saveItem: function(ev) {
            	var data = Backbone.Syphon.serialize($('form'));
            	this.model.set(data);
            	this.model.set('unsavedChanges', true);
            	App.unsavedChanges = true;
            	if (!_.isUndefined(this.options) && !_.isNull(this.options) && !_.isUndefined(this.options.save) && _.isFunction(this.options.save)) {
            		this.options.save(this.model);
            	}
            	var dialog = this.$el.closest('.modal');
    			dialog.modal('hide');
    			return false;
            },
        	
        	initialize:function (options) {
            },
            
            onRender: function (ev) {
            	
            }
        });
        
        return UssdCommandView;
});